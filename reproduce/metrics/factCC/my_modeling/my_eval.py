...
def evaluate_model_my(args, model, tokenizer, prefix=""):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_task_names = (args.task_name,)
    # eval_outputs_dirs = (args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                preds = logits.detach().cpu().numpy()
                # text_ids = inputs['guids'].detach().cpu().numpy()
                # out_label_ids = inputs['labels'].detach().cpu().numpy()
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                # text_ids = np.append(text_ids,inputs['guids'].detach().cpu().numpy(),axis=0)
                # out_label_ids = np.append(out_label_ids, inputs['labels'].detach().cpu().numpy(), axis=0)
        ...
        # result["loss"] = eval_loss
        # results.update(result)
        # result = list(zip(text_ids.tolist(),preds.tolist()))
        result = preds.tolist()

        output_eval_file = os.path.join(eval_output_dir, "eval_model_result.json")
        with open(output_eval_file, "w") as writer:
            logger.info("***** Eval results {} *****".format(prefix))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    # all_guids = torch.tensor([f.guid for f in features], dtype=torch.long)  # my_add
    ...
    # dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,
    #                         all_ext_mask, all_ext_start_ids, all_ext_end_ids,
    #                         all_aug_mask, all_aug_start_ids, all_aug_end_ids, all_guids)
    return dataset
...
def main():
    ...
    # Training
    if args.do_train:
        # results = evaluate(args, model, tokenizer)  # my_add
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            result = evaluate_model_my(args, model, tokenizer, prefix=global_step)
